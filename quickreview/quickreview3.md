...
Tampilan dari kode tersebut secara [visual](https://cscircles.cemc.uwaterloo.ca/visualize#code=class+Buku%3A%0A++++total_buku+%3D+0%0A%0A++++def+__init__(self,+judul,+stok)%3A%0A++++++++self.judul+%3D+judul%0A++++++++self._status+%3D+'Tersedia'%0A++++++++Buku.total_buku+%2B%3D+1%0A++++++++self.stok+%3D+stok%0A%0A++++%40property%0A++++def+stok(self)%3A%0A++++++++return+self.__stok%0A%0A++++%40stok.setter%0A++++def+stok(self,+nilai)%3A%0A++++++++if+not+isinstance(nilai,+int)+or+nilai+%3C+0%3A%0A++++++++++++raise+ValueError('Stok+harus+bilangan+bulat+tidak+negatif.')%0A++++++++self.__stok+%3D+nilai%0A++++++++self._status+%3D+'Tersedia'+if+nilai+%3E+0+else+'Habis'%0A%0A++++%40property%0A++++def+tersedia(self)%3A%0A++++++++return+self.stok+%3E+0%0A%0A++++def+pinjam(self)%3A%0A++++++++if+not+self.tersedia%3A%0A++++++++++++return+False%0A++++++++self.stok+-%3D+1%0A++++++++return+True%0A%0Abuku+%3D+Buku('Belajar+OOP',+2)%0Aprint('Stok+awal%3A',+buku.stok)%0Aprint('Pinjam%3A',+buku.pinjam())%0Aprint('Stok+akhir%3A',+buku.stok)%0Aprint('Tersedia%3A',+buku.tersedia)%0Aprint('Status%3A',+buku._status)%0Aprint('Total+buku%3A',+Buku.total_buku)%0A&mode=display&raw_input=&curInstr=26)
